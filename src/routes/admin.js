...
function createAdminRouter(bookStore) {
  ...
  router.get('/', (req, res) => {
    res.send(adminDashboardPage({ books: bookStore.list() }));
  });
  ...
  router.post('/books/:id', (req, res) => {
    const existing = bookStore.get(req.params.id);
    if (!existing) return res.status(404).send('Book not found');
    const error = validateBook(req.body);
    if (error) {
      return res.status(400).send(adminBookFormPage({ book: { ...req.body, id: req.params.id }, isNew: false, error }));
    }
    bookStore.update(req.params.id, req.body);
    res.redirect('/admin');
  });

  router.post('/books/:id/delete', (req, res) => {
    bookStore.delete(req.params.id);
    res.redirect('/admin');
  });
